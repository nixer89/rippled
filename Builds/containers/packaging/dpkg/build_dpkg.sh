...
git archive --format tar.gz --prefix xrpld-${XRPLD_DPKG_VERSION}/ -o ../xrpld-${XRPLD_DPKG_VERSION}.tar.gz HEAD
cd ..
# dpkg debmake would normally create this link, but we do it manually
ln -s ./xrpld-${XRPLD_DPKG_VERSION}.tar.gz xrpld_${XRPLD_DPKG_VERSION}.orig.tar.gz
tar xvf xrpld-${XRPLD_DPKG_VERSION}.tar.gz
cd xrpld-${XRPLD_DPKG_VERSION}
cp -pr ../debian .
...
xrpld (${XRPLD_DPKG_FULL_VERSION}) unstable; urgency=low
...
# copy artifacts
cp xrpld-dev_${XRPLD_DPKG_FULL_VERSION}_amd64.deb ${PKG_OUTDIR}
cp xrpld_${XRPLD_DPKG_FULL_VERSION}_amd64.deb ${PKG_OUTDIR}
cp xrpld_${XRPLD_DPKG_FULL_VERSION}.dsc ${PKG_OUTDIR}
# dbgsym suffix is ddeb under newer debuild, but just deb under earlier
cp xrpld-dbgsym_${XRPLD_DPKG_FULL_VERSION}_amd64.* ${PKG_OUTDIR}
cp xrpld_${XRPLD_DPKG_FULL_VERSION}_amd64.changes ${PKG_OUTDIR}
cp xrpld_${XRPLD_DPKG_FULL_VERSION}_amd64.build ${PKG_OUTDIR}
cp xrpld_${XRPLD_DPKG_VERSION}.orig.tar.gz ${PKG_OUTDIR}
cp xrpld_${XRPLD_DPKG_FULL_VERSION}.debian.tar.xz ${PKG_OUTDIR}
# buildinfo is only generated by later version of debuild
if [ -e xrpld_${XRPLD_DPKG_FULL_VERSION}_amd64.buildinfo ] ; then
    cp xrpld_${XRPLD_DPKG_FULL_VERSION}_amd64.buildinfo ${PKG_OUTDIR}
fi

cat xrpld_${XRPLD_DPKG_FULL_VERSION}_amd64.changes
# extract the text in the .changes file that appears between
#    Checksums-Sha256:  ...
# and
#    Files: ...
awk '/Checksums-Sha256:/{hit=1;next}/Files:/{hit=0}hit' \
    xrpld_${XRPLD_DPKG_VERSION}-1_amd64.changes | \
        sed -E 's!^[[:space:]]+!!' > shasums
DEB_SHA256=$(cat shasums | \
    grep "xrpld_${XRPLD_DPKG_VERSION}-1_amd64.deb" | cut -d " " -f 1)
DBG_SHA256=$(cat shasums | \
    grep "xrpld-dbgsym_${XRPLD_DPKG_VERSION}-1_amd64.*" | cut -d " " -f 1)
DEV_SHA256=$(cat shasums | \
    grep "xrpld-dev_${XRPLD_DPKG_VERSION}-1_amd64.deb" | cut -d " " -f 1)
SRC_SHA256=$(cat shasums | \
    grep "xrpld_${XRPLD_DPKG_VERSION}.orig.tar.gz" | cut -d " " -f 1)
...
echo "dpkg_version=${XRPLD_DPKG_VERSION}" >> ${PKG_OUTDIR}/build_vars
echo "dpkg_full_version=${XRPLD_DPKG_FULL_VERSION}" >> ${PKG_OUTDIR}/build_vars
